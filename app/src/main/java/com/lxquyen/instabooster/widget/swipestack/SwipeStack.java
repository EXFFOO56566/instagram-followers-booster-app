...
public class SwipeStack extends ViewGroup {
    ...
    private void reorderItems() {
        for (int x = 0; x < getChildCount(); x++) {
            ...
            int newPositionX = (getWidth() - childView.getMeasuredWidth() + distanceToViewAbove) / 2;
            int newPositionY = distanceToViewAbove + getPaddingTop();

            childView.layout(
                    newPositionX,
                    getPaddingTop(),
                    newPositionX + childView.getMeasuredWidth() - distanceToViewAbove,
